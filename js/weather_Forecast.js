...
searchButton.addEventListener("click", async function () {
    ...
    try {
        const response = await fetch(API_URL + encodeURIComponent(stringWithout_Accents_And_Diacritcs))
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}`)
        }

        const data = await response.json()
        dataProcess(data, stringWithout_Accents_And_Diacritcs, 0)

        localStorage.setItem("dataSave" + dataCount, JSON.stringify(data))
        dataCount++
    } catch (error) {
        console.error("lỗi khi fetch API", error)
        alert("Không thể tải dữ liệu thời tiết cho thành phố này.")
    }
})
